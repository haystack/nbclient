...
function forEachNodeInRange(range, callback) {
    ...
    const nodeIter = root.ownerDocument.createNodeIterator(root, NodeFilter.SHOW_ALL, null /* filter */, false /* expandEntityReferences */)
...
function getTextBoundingBoxes(range) {
    ...
    forEachNodeInRange(range, node => {
        if (node.nodeType === Node.TEXT_NODE
            ...
            && !node.parentNode.nodeName.includes('NB')) { // Ignore any nb custom tags like 'nb-innotation-inline'
            textNodes.push(node)
        }
    })
